<?php 
 ...
 session_start();
    ...
    if (!($user -> LoggedIn()) || !($user -> notBanned($odb))){
		die(error('SUCK MY DICK SKID!'));
		sleep(3);
		header('location: logout.php');
    }
	if (!$user->hasMembership($odb)){
		die(error('You dont have plan!'));
		$row['mbt'] = 0;
		$row['concurrents'] = 0;
    ...
    if ($type == 'start' || $type == 'renew'){
        ...
        if($cooldowncheck->fetchColumn() > 0){
			...
			$SQL = $odb -> prepare("SELECT `plans`.`vip` FROM `plans` LEFT JOIN `users` ON `users`.`membership` = `plans`.`ID` WHERE `users`.`username` = ?");
			$SQL -> execute(array($_SESSION['username']));
			$vips = $SQL -> fetchColumn(0)+$avip;
			if ($vips == 1){
				//FIXED, RIPx IS THE BEST ;) 
			}elseif($vips == 2){
				//FIXED, RIPx IS THE BEST ;) 
			}else{
				die(error('You need vip to use this!'));
			}
			}
		...
		if($method == "OVH"){
			    if ($time > 200){
			       die(error('Your max boot time ovh is 200!! .'));
		        }
		}
		if($method == "OVH"){
			    if ($totalservers > 1){
			       die(error('Max servers for this method is 1!! .'));
		        }
		}
