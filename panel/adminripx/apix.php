...
if (isset($_POST['delete'])){
	...
	if($_SESSION['username'] == "OWNER"){								
		...
		$error = error('only owner can delete the methods ;) ');
		}
		if (empty($error)){
		$delete = $_POST['delete'];
	...
	if (isset($_POST['deleteapi'])){
		...
		$error = error('only owner can delete the apis');
		}
...
<form method="post">
							<?php
								...
								while($getInfo = $SQLGetMethods -> fetch(PDO::FETCH_ASSOC)){
								    ...
								    $apisecurity = ' <span class="badge" style="background: linear-gradient(135deg, #eab000 0, rgba(255, 202, 40, 0.37) 100%)!important;">only owner can see api links</span> ';
									}
									 echo '<tr>
